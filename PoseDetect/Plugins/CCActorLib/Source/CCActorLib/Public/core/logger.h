...
namespace std
{
    ...
    inline std::basic_ostream<char, std::char_traits<char>> & operator<<(std::basic_ostream<char, std::char_traits<char>>&& _Ostr, const wchar_t * _Val)
    {
        if (!_Ostr.good())
            return _Ostr;
        return _Ostr << core::ucs2_u8(_Val);
    }
    inline std::basic_ostream<char, std::char_traits<char>> & operator<<(std::basic_ostream<char, std::char_traits<char>>& _Ostr, const wchar_t * _Val)
    {
        if (!_Ostr.good())
    ...
    inline std::basic_ostream<char, std::char_traits<char>> & operator<<(std::basic_ostream<char, std::char_traits<char>>&& _Ostr, const std::u16string & _Val)
    {
        if (!_Ostr.good())
            return _Ostr;
        return _Ostr << core::ucs2_u8((const wchar_t *)_Val.c_str(), _Val.length());
    }
    inline std::basic_ostream<char, std::char_traits<char>> & operator<<(std::basic_ostream<char, std::char_traits<char>>& _Ostr, const std::u16string & _Val)
    {
        if (!_Ostr.good())
...
namespace core
{
    ...
    class CCACTORLIB_API logger_stream : public std::ostream, private std::streambuf
    {
    public:
    ...
    class CCACTORLIB_API logger
    {
    public:
        logger();
        logger(std::wstring path, log_e lg);
        ...
        error_e open(std::wstring path);
        void close();
        void flush();

		void SaveBKlog(std::wstring path);//log文件超出大小后拷贝到log.bk，log.bk超出大小后拷贝到log.bk.bk,log.bk.bk超出大小后删除
    ...
    class CCACTORLIB_API global_logger
    {
    public:
        global_logger();
        global_logger(std::wstring path, log_e lg);
        ~global_logger();

        static void start(std::wstring path, log_e lg);
        static void stop();
