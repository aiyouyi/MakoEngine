...
{
    JceDecodeMismatch(const std::string & s) : 
	JceDecodeException(s) {
    ...
    void read(std::string& s, uint8_t tag, bool isRequire = true)
    {
        if(skipToTag(tag)){
            ...
            case DataHead::eString4:
                {
                    ...
                    if(len > JCE_MAX_STRING_LENGTH){
                        char s[128];
                        snprintf(s, sizeof(s), "invalid std::string size, tag: %d, size: %d", tag, len);
#ifndef NOT_USE_EXCEPTION_FOR_ANDROID
            ...
            default:
                {
                    char s[64];
                    snprintf(s, sizeof(s), "read 'std::string' type mismatch, tag: %d, get type: %d.", tag, h.getType());
#ifndef NOT_USE_EXCEPTION_FOR_ANDROID
						throw JceDecodeMismatch(s);
#endif
                }
    ...
    void read(std::vector<T, Alloc>& v, uint8_t tag, bool isRequire = true)
    {
        if(skipToTag(tag)){
            ...
            default:
                {
                    char s[64];
                    snprintf(s, sizeof(s), "read 'std::vector' type mismatch, tag: %d, get type: %d.", tag, h.getType());
					#ifndef NOT_USE_EXCEPTION_FOR_ANDROID
					throw JceDecodeMismatch(s);
					#endif
                }
    ...
    void read(T* v, const UInt32 len, UInt32 & readLen, uint8_t tag, bool isRequire = true)
    {
        if(skipToTag(tag)){
            ...
            default:
                {
                    char s[64];
                    snprintf(s, sizeof(s), "read 'std::vector struct' type mismatch, tag: %d, get type: %d.", tag, h.getType());
					#ifndef NOT_USE_EXCEPTION_FOR_ANDROID
					throw JceDecodeMismatch(s);
					#endif
                }
    ...
    void write(const std::string& s, uint8_t tag)
    {
        if(s.size() > 255){
            if(s.size() > JCE_MAX_STRING_LENGTH){
                char ss[128];
                snprintf(ss, sizeof(ss), "invalid std::string size, tag: %d, size: %u", tag, (uint32_t)s.size());
                #ifndef NOT_USE_EXCEPTION_FOR_ANDROID
				throw JceDecodeInvalidValue(s);
				#endif
            }
