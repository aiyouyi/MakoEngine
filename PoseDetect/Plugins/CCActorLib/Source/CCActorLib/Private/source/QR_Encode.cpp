...
BOOL CQR_Encode::EncodeData(int nLevel, int nVersion, BOOL bAutoExtent, int nMaskingNo, LPCWSTR lpsSource, int ncSource)
{
	int i, j;
	...
	int ncLength = ncSource > 0 ? ncSource : lstrlenW(lpsSource);
...
int CQR_Encode::GetEncodeVersion(int nVersion, LPCWSTR lpsSource, int ncLength)
{
...
BOOL CQR_Encode::EncodeSourceData(LPCWSTR lpsSource, int ncLength, int nVerGroup)
{
