...
//--------------------------------------------------------------------------------------
static HRESULT LoadTextureDataFromFile( __in_z const char* szFileName, BYTE** ppHeapData,
                                        DDS_HEADER** ppHeader,
...
static HRESULT LoadTextureDataFromBuffer(__in_z BYTE* data, int nSize, BYTE** ppHeapData,
	DDS_HEADER** ppHeader,
	BYTE** ppBitData, UINT* pBitSize)
{
	// open the file
	...
	// Need at least enough data to fill the header and magic number to be a valid DDS
	if (nSize < (sizeof(DDS_HEADER) + sizeof(DWORD)))
	{
		return E_FAIL;
	}
	...
	*ppHeapData = data;
	...
	{
		// Must be long enough for both headers and magic value
		if (nSize < (sizeof(DDS_HEADER) + sizeof(DWORD) + sizeof(DDS_HEADER_DXT10)))
		{
			return E_FAIL;
		}
...
//--------------------------------------------------------------------------------------
HRESULT CreateDDSTextureFromFile( ID3D11Device* pDev, const char* szFileName, ID3D11ShaderResourceView** ppSRV, bool bSRGB )
{
...
HRESULT CreateDDSTextureFromBuffer(ID3D11Device* pDev, BYTE* pData, int nDataSize, ID3D11ShaderResourceView** ppSRV, SDDSInfo *ddsInfo, bool sRGB)
{
	if (ddsInfo != NULL)
	{
		memset(ddsInfo, 0, sizeof(SDDSInfo));
	}
	...
	HRESULT hr = LoadTextureDataFromBuffer(pData, nDataSize, &pHeapData, &pHeader, &pBitData, &BitSize);
	if (FAILED(hr))
	{
		return hr;
	}
	if (ddsInfo != NULL)
	{
		ddsInfo->iWidth = pHeader->dwWidth;
		ddsInfo->iHeight = pHeader->dwHeight;
		ddsInfo->iMipCount = pHeader->dwMipMapCount;
	}
