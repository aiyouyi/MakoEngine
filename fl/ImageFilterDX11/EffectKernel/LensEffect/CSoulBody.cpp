...
void * CSoulBody::Clone()
{
    CSoulBody* result = new CSoulBody();
    *result = *this;
    return result;
}

bool CSoulBody::ReadConfig(XMLNode & childNode, HZIP hZip, char *pFilePath)
{
...
bool CSoulBody::ReadConfig(XMLNode & childNode, const std::string & path)
{
...
bool CSoulBody::Prepare()
{
	CCVetexAttribute pAttribute[] =
	{
		{VERTEX_ATTRIB_POSITION, FLOAT_C2},
	};
	std::string  path = m_resourcePath + "/Shader/LensEffect/Soulbody.fx";
...
void CSoulBody::Render(BaseRenderParam &RenderParam)
{
	if (m_alpha < 0.001f)
	{
		return;
...
bool CSoulBody::ReadConfig(XMLNode & childNode, HZIP hZip, char * pFilePath, const std::string & path)
{
...
bool CSoulBody::WriteConfig(std::string &tempPath, XMLNode &root, HZIP dst, HZIP src)
{
...
void CSoulBody::SetEffectCount(int count)
{
	m_Loop = count;
