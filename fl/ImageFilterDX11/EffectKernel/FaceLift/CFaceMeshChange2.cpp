...
CFaceMeshChange2::CFaceMeshChange2()
{
...
void CFaceMeshChange2::Resize(int nWidth, int nHeight)
{
	m_nWidth = nWidth;
	m_nHeight = nHeight;
...
void CFaceMeshChange2::RenderMaterial(int nIndex,Vector2 *pFacePoint)
{
	...
	{
		m_rectVerticeBuffer[nIndex] = DXUtils::CreateVertexBuffer((float*)pVertLeft, 4, 4);
	}
	else
	{
...
void * CFaceMeshChange2::Clone()
{
	CFaceMeshChange2* result = new CFaceMeshChange2();
	*result = *this;
	return result;
}

bool CFaceMeshChange2::ReadConfig(XMLNode & childNode, HZIP hZip, char * pFilePath)
{
	if (!childNode.isEmpty())
	{
		partMLS = new CFaceSPMLS();
		partMLS->ReadConfig(childNode, hZip, pFilePath);
		...
		const char *szChangeAlpha = nodeModel.getAttribute("changeAlpha");
		if (szChangeAlpha!=NULL)
		{
			m_changelpha = atof(szChangeAlpha);
		}
...
bool CFaceMeshChange2::Prepare()
{
	string path = m_resourcePath + "/Shader/face2dTexture.fx";

	partMLS->SetResourcePath(m_resourcePath);
	partMLS->Prepare();
...
void CFaceMeshChange2::Render(BaseRenderParam & RenderParam)
{
...
//	DXUtils::SetRasterizerState(false);
	...
	CalFaceChangePoint(RenderParam);
...
void CFaceMeshChange2::CalFaceChangePoint(BaseRenderParam & RenderParam)
{
	int nfps = m_nFPS;
	...
	partMLS->m_alpha = m_changelpha * alpha;
	partMLS->Render(RenderParam);
