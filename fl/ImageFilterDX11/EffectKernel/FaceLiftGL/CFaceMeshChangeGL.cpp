...
CFaceMeshChangeGL::CFaceMeshChangeGL()
{
...
void CFaceMeshChangeGL::Release()
{
	m_pShader.reset();
	m_pShaderMat.reset();
...
void CFaceMeshChangeGL::Resize(int nWidth, int nHeight)
{
	m_nWidth = nWidth;
	m_nHeight = nHeight;
}

void CFaceMeshChangeGL::RenderMaterial(int nIndex, Vector2 *pFacePoint)
{
	...
	Vector2 pVertLeft[4], pVertRight[4];

	for (int i = 0; i < 4; i++)
	{
		Vector2 Dist = pRotate * (m_VTLeft[i] - pStandLeftPoint)*Vector2(m_nWidth, m_nWidth)*pScale.x;
		pVertLeft[i] = (Dist + pFacePoint[5])* Vector2(1.0 / m_nWidth, 1.0 / m_nHeight);

		Dist = pRotate * (m_VTRight[i] - pStandRightPoint)*Vector2(m_nWidth, m_nWidth)*pScale.x;
		pVertRight[i] = (Dist + pFacePoint[27])* Vector2(1.0 / m_nWidth, 1.0 / m_nHeight);

		pVertLeft[i] = pVertLeft[i] * 2.0 - Vector2(1.0, 1.0);
		pVertRight[i] = pVertRight[i] * 2.0 - Vector2(1.0, 1.0);
	}
	...
	m_pShaderMat->SetUniform1i("inputImageTexture", 0);

	m_pShaderMat->SetVertexAttribPointer("a_position", 2, GL_FLOAT, false, 0, (float*)pVertLeft);
	m_pShaderMat->SetVertexAttribPointer("a_texcoord", 2, GL_FLOAT, false, 0, (float*)m_VTLeft);
	glDrawElements(GL_TRIANGLES, 2 * 3, GL_UNSIGNED_SHORT, index);

	auto pMaterialView2 = m_2DAnimation[1]->GetTex(m_runTime);
	pMaterialView2->Bind(0);
	m_pShaderMat->SetUniform1i("inputImageTexture", 0);

	m_pShaderMat->SetVertexAttribPointer("a_position", 2, GL_FLOAT, false, 0, (float*)pVertRight);
	m_pShaderMat->SetVertexAttribPointer("a_texcoord", 2, GL_FLOAT, false, 0, (float*)m_VTRight);
	glDrawElements(GL_TRIANGLES, 2 * 3, GL_UNSIGNED_SHORT, &index[0]);

	glDisable(GL_BLEND);
}


void * CFaceMeshChangeGL::Clone()
{
	CFaceMeshChangeGL* result = new CFaceMeshChangeGL();
	*result = *this;
	return result;
}

bool CFaceMeshChangeGL::ReadConfig(XMLNode & childNode, HZIP hZip, char * pFilePath)
{
...
bool CFaceMeshChangeGL::Prepare()
{
	m_pShader = std::make_shared<CCProgram>();
	std::string  vspath = m_resourcePath + "/Shader/faceEffectMeshChange.vs";
	std::string  fspath = m_resourcePath + "/Shader/faceEffectMeshChange.fs";
	m_pShader->CreatePorgramForFile(vspath.c_str(), fspath.c_str());

	m_pShaderMat = std::make_shared<CCProgram>();
	std::string  vspath_tex = m_resourcePath + "/Shader/face2dTexture.vs";
	std::string  fspath_tex = m_resourcePath + "/Shader/face2dTexture.fs";
	m_pShaderMat->CreatePorgramForFile(vspath_tex.c_str(), fspath_tex.c_str());
...
bool CFaceMeshChangeGL::DrawTriLineToFBO(GLfloat* points, int count, unsigned short* indexs)
{
...
void CFaceMeshChangeGL::Render(BaseRenderParam& RenderParam)
{
	if (RenderParam.GetFaceCount() == 0)
	{
		return;
	}
	m_runTime = RenderParam.runtime;
	...
	for (int faceIndex = 0; faceIndex < nFaceCount; faceIndex++)
	{
		...
		m_pShader->SetVertexAttribPointer("a_position", 2, GL_FLOAT, false, 0, (float*)pDstPoint171);
		m_pShader->SetVertexAttribPointer("a_texcoord", 2, GL_FLOAT, false, 0, (float*)pSrcpoint171);
		glDrawElements(GL_TRIANGLES, FaceMesh->nTriangle * 3, GL_UNSIGNED_SHORT, FaceMesh->pTriangleIndex);
		//DrawTriLineToFBO((float*)pDstPoint171, FaceMesh->nTriangle, FaceMesh->pTriangleIndex);
...
void CFaceMeshChangeGL::CalFaceChangePoint(Vector2 * pSrcPoint118, Vector2 * pDstPoint118)
{
	// 	float m_alpha[] =
	// 	{
