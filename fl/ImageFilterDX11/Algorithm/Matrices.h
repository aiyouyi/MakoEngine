...
public:
	...
	friend Vector2 operator*(const Vector2& vec, const Matrix4& m); // pre-multiplication
...
inline Vector2 Matrix4::operator*(const Vector2& rhs) const
{
	return Vector2(m[0]*rhs.x + m[1]*rhs.y + + m[3],
...
{
	return Vector2(v.x*m[0] + v.y*m[4] + m[12],v.x*m[1] + v.y*m[5] + m[13]);
}
