...
{
	float CC3DAnimate::AnimationSLerp = 0.5f;
	...
	void CC3DAnimate::ReadSkeletonAnimateData(CC3DModel* pModel, CC3DSkeleton* pSkeleton, const char* pAnimateData)
	{
		if (pSkeleton == NULL || pAnimateData == NULL)
		{
			return;
		}
		int nOffest;
		memcpy(&m_AnimateAllTime, pAnimateData, sizeof(float));
		nOffest = sizeof(float);
		pAnimateData += nOffest;

		int nChannels;
		memcpy(&nChannels, pAnimateData, sizeof(int));
		nOffest = sizeof(int);
		pAnimateData += nOffest;
		...
		for (int i = 0; i < nChannels; i++)
		{
			CC3DAnimationChannelInfo* pInfo = new CC3DAnimationChannelInfo();
			int nType;
			memcpy(&nType, pAnimateData, sizeof(int));
			nOffest = sizeof(int);
			pAnimateData += nOffest;
			...
			{
				int nLen;
				memcpy(&nLen, pAnimateData, sizeof(int));
				nOffest = sizeof(int);
				pAnimateData += nOffest;

				char* boneName = new char[nLen + 1];
				boneName[nLen] = '\0';
				memcpy(boneName, pAnimateData, nLen);
				nOffest = nLen;
				pAnimateData += nOffest;
				...
				memcpy(pData, pAnimateData, nLen * sizeof(float));
				nOffest = nLen * sizeof(float);
				pAnimateData += nOffest;
				...
				{
					pData = new uint8[nLen * sizeof(Vector4)];
					memcpy(pData, pAnimateData, nLen * sizeof(Vector4));
					nOffest = nLen * sizeof(Vector4);
					pAnimateData += nOffest;
				...
				{
					pData = new uint8[nLen * sizeof(Vector3)];
					memcpy(pData, pAnimateData, nLen * sizeof(Vector3));
					nOffest = nLen * sizeof(Vector3);
					pAnimateData += nOffest;
					pInfo->pOutputAnimateValue = (float*)pData;
				}
				m_pData.push_back(pData);
			...
			else
			{
				...
				char* MeshName = new char[nLen + 1];
				MeshName[nLen] = '\0';
				memcpy(MeshName, pAnimateData, nLen);
				nOffest = nLen;
				pAnimateData += nOffest;
				...
				pAnimateData += nOffest;
				int nBS = 0;
				memcpy(&nBS, pAnimateData, sizeof(int));
				nOffest = sizeof(int);
				pAnimateData += nOffest;
				pInfo->nKeyFrame = nLen;
				pInfo->nOutCount = nLen * nBS;
				...
				m_pData.push_back(pData);
				pData = new uint8[nLen * nBS * sizeof(float)];
				memcpy(pData, pAnimateData, nLen * nBS * sizeof(float));
				nOffest = nLen * nBS * sizeof(float);
				pAnimateData += nOffest;
				pInfo->pOutputAnimateValue = (float*)pData;
	...
	void CC3DAnimate::play(float fSecond, CC3DModel* pModel, CC3DSkeleton* pSkeleton, AnimBlendInfo& blendInfo)
	{
		int nChannels = m_ChannelInfo.size();
		for (int i = 0; i < nChannels; i++)
		{
			...
			if (pInfo->eType == TARGET_WEIGHT)
			{
				playBlendShapeWithBlend(fSecond, pModel, pInfo, &blendInfo);
			}
			else
			{
				playSkeletonWithBlend(fSecond, pSkeleton, pModel, pInfo, &blendInfo);
			}
	...
	void CC3DAnimate::LoadAnimateJson(const std::string& fileName, CC3DSkeleton* pSkeleton)
	{
		if (!m_BodyDriveConfig)
		{
			m_BodyDriveConfig = std::make_shared<CC3DBodyDriveConfig>();
		}
		m_BodyDriveConfig->LoadConfig(fileName, pSkeleton);
		m_TStart = std::chrono::high_resolution_clock::now();
	}

	void CC3DAnimate::playByConfig(CC3DModel* pModel, CC3DSkeleton* pSkeleton)
	{
		if (!m_BodyDriveConfig || !pSkeleton)
		{
			return;
		}
		...
		if (Delta < (1000.0f / 30.f))
		{
			return;
		}
		m_TStart = m_TEnd;

		const auto& AnimateList = m_BodyDriveConfig->GetBoneInfoList();

		for (auto ItBoneInfoList = AnimateList[m_playIndex].begin(); ItBoneInfoList != AnimateList[m_playIndex].end(); ++ItBoneInfoList)
		{
			const auto& BoneInfo = *ItBoneInfoList;
			int32_t NodeId = -1;
			...
			NodeId = pSkeleton->m_BoneMap.find(BoneInfo.BoneName)->second;
			{
				...
				if (pBoneInfo)
				{
					...
					CC3DMath::CC3DQuaternion InitRotation(BoneInfo.InitQuat.x, BoneInfo.InitQuat.y, BoneInfo.InitQuat.z, BoneInfo.InitQuat.w);
					CC3DMath::CC3DQuaternion TargetRotation(BoneInfo.Quat.x, BoneInfo.Quat.y, BoneInfo.Quat.z, BoneInfo.Quat.w);

					TargetRotation = TargetRotation * InitRotation;

					CC3DMath::CC3DQuaternion LastQuat = CC3DMath::CC3DQuaternion(pBoneInfo->TargetRotation.x, pBoneInfo->TargetRotation.y, pBoneInfo->TargetRotation.z, pBoneInfo->TargetRotation.w);

					CC3DMath::CC3DQuaternion NewQuat;
					CC3DMath::CC3DQuaternion::slerp(LastQuat, TargetRotation, CC3DAnimate::AnimationSLerp, &NewQuat);
					pBoneInfo->TargetRotation = Vector4(NewQuat.x, NewQuat.y, NewQuat.z, NewQuat.w);
				}
			}
		}

		m_playIndex = ++m_playIndex % AnimateList.size();
	...
	void CC3DAnimate::playBlendShapeWithBlend(float fSecond, CC3DModel* pModel, CC3DAnimationChannelInfo* pInfo, AnimBlendInfo* blendInfo /*= nullptr*/)
	{
		uint32 nNodeID = pInfo->nNodeID;
	...
	void CC3DAnimate::playSkeletonWithBlend(float fSecond, CC3DSkeleton* pSkeleton, CC3DModel* pModel, CC3DAnimationChannelInfo* pInfo, AnimBlendInfo* blendInfo /*= nullptr*/)
	{
		int nNodeID = pInfo->nNodeID;
		if (pSkeleton != NULL && pSkeleton->m_NodeBoneMap.find(nNodeID) != pSkeleton->m_NodeBoneMap.end())
		{
			CC3DBoneNodeInfo* pBoneInfo = pSkeleton->m_BoneNode[pSkeleton->m_NodeBoneMap[nNodeID]];
			if (pBoneInfo && blendInfo)
			{
				if (!pBoneInfo->IsChildOfMaskBone(blendInfo->maskBoneName))
				{
					return;
				}
			}
			...
			if (nKeyL >= 0)
			{
				if (pInfo->eType == TARGET_ROTATE)
				{
					...
					Vector4* pR = (Vector4*)pInfo->pOutputAnimateValue;
					//pBoneInfo->TargetRotation = pL[nKeyL] * (1.0 - alpha) + pR[nKeyR] * alpha;
					CC3DUtils::QuaternionInterpolate(boneInfo.Rotation, pL[nKeyL], pR[nKeyR], alpha);
					boneInfo.EnableRotate = true;
				}
				else if (pInfo->eType == TARGET_SCALE)
				{
					Vector3* pL = (Vector3*)pInfo->pOutputAnimateValue;
					Vector3* pR = (Vector3*)pInfo->pOutputAnimateValue;
					boneInfo.Scale = pL[nKeyL] * (1.0 - alpha) + pR[nKeyR] * alpha;
					boneInfo.EnableScale = true;
				}
				else if (pInfo->eType == TARGET_TRANSLATE)
				{
					Vector3* pL = (Vector3*)pInfo->pOutputAnimateValue;
					Vector3* pR = (Vector3*)pInfo->pOutputAnimateValue;
					boneInfo.Translate = pL[nKeyL] * (1.0 - alpha) + pR[nKeyR] * alpha;
					boneInfo.EnableTranslate = true;
				}
				if (blendInfo)
				{
					blendInfo->boneNodeMap[nNodeID] = boneInfo;
				}
		...
		else
		{
			...
			if (nKeyL >= 0)
			{
				if (pInfo->eType == TARGET_ROTATE)
				{
					...
					Vector4* pR = (Vector4*)pInfo->pOutputAnimateValue;
					//pBoneInfo->TargetRotation = pL[nKeyL] * (1.0 - alpha) + pR[nKeyR] * alpha;
					CC3DUtils::QuaternionInterpolate(nodeInfo.Rotation, pL[nKeyL], pR[nKeyR], alpha);
					nodeInfo.EnableRotate = true;
				}
				else if (pInfo->eType == TARGET_SCALE)
				{
					Vector3* pL = (Vector3*)pInfo->pOutputAnimateValue;
					Vector3* pR = (Vector3*)pInfo->pOutputAnimateValue;
					nodeInfo.Scale = pL[nKeyL] * (1.0 - alpha) + pR[nKeyR] * alpha;
					nodeInfo.EnableScale = true;
				}
				else if (pInfo->eType == TARGET_TRANSLATE)
				{
					Vector3* pL = (Vector3*)pInfo->pOutputAnimateValue;
					Vector3* pR = (Vector3*)pInfo->pOutputAnimateValue;
					nodeInfo.Translate = pL[nKeyL] * (1.0 - alpha) + pR[nKeyR] * alpha;
					nodeInfo.EnableTranslate = true;
				}
				if (blendInfo)
				{
					blendInfo->modelNodeMap[nNodeID] = nodeInfo;
				}
			}
			hasModelAnimate = true;
		}
