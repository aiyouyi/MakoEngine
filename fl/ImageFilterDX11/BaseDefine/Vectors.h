...
#ifndef _CC_VECTORS_H_DEF
#define _CC_VECTORS_H_DEF
...
struct Vector2
{
	...
	Vector2(float a) noexcept :x(a), y(a) {};
...
//Structure that holds ParticleQuad information
class ParticleQuad{
public :
...
struct Vector3
{
	...
	Vector3(float a) noexcept :x(a), y(a), z(a) {};
...
struct Vector4
{
	...
	Vector4(float a) noexcept :x(a), y(a), z(a), w(w) {};
...
namespace math
{
	///scalar
	template<typename T>
	T vmin(T a, T b) noexcept {
		return a < b ? a : b;
	}

	template<typename T>
	T vmax(T a, T b) noexcept {
		return a > b ? a : b;
	}

	template<typename T>
	T clamp(T v, T min, T max) noexcept {
		assert(min <= max);
		return T(math::vmin(max, math::vmax(min, v)));
	}
	...
	template<typename T>
	T mix(T x, T y, T a) noexcept {
		return  x * (T(1) - a) + y * a;
	}

	template<typename T>
	T lerp(T x, T y, T a) noexcept {
		return mix(x, y, a);
	}

	template<typename T>
	T smoothstep(T e0, T e1, T x) noexcept {
	...
	//// implement
	//template<>
	//float mix(float x, float y, float a) noexcept
	//{
	//	return x * (1.0f - a) + y * a;
	//}
